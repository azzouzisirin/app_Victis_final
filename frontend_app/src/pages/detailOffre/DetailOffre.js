...
const [heureformation, setheureformation] = useState();
  ...
  const NumDevis =localStorage.getItem("numDevis")
  const numSession =localStorage.getItem("numSession")
...
if(selectedOptionRaison&&selectedOptionCateg){
  ...
  useEffect(() => {
      if(DesignationFormation){
       fetch("/formation/findByCateg/"+DesignationFormation.value)
       .then((res) => res.json())
       .then((data) => {
         const userData = data.map((item) => ({
           label: item.type,
...
if(IdFormateur){
  ...
  var typeformation=''
   ...
   try{
    ...
    if(DesignationFormation.value){
      nomFormation=DesignationFormation.value
    }else{
      nomFormation=DesignationFormation
    }
    if(typeFormation.value){
      ...
      const create = await axios.post(BASE_URL+"/newfolder/"+nomDossier,{
        ...
        const res = await axios.put(BASE_URL+"/offre/"+id, {
          ...
          prixNet       :PrixAvecTva,
          typeFormation:typeFormation.value,
          designiationFormation:DesignationFormation.value,
   ...
   const c=Number(Panier)
    ...
    useEffect(() => {
      ...
      if(selectedTypeFormation=="En inter-enterprise"){
        if(checkAuilleur==true){
          setlieuFormation(listBureau.adresse_1)

         }
         if(checkEng==true){
          setlieuFormation("8 rue Honoré de Balzac 37000 Tours")
      ...
      }, [selectedTypeFormation,Adresse_1,typeBureau,checkAuilleur]);
     ...
     const c=Number(fraisRestau)
      ...
      useEffect(() => {
        ...
        if(selectedOptionRaisonFinanceur){
          fetch("/financeur/findByRaison/"+selectedOptionRaisonFinanceur.value)
          .then((res) => res.json())
          .then((data) => { 
            const userData = data.map((item) => ({
              label: item.username,
    ...
    useEffect(() => {
     ...
     if(selectedOptionRaison){
      fetch("/client/findByRaison/"+selectedOptionRaison.value)
      .then((res) => res.json())
      .then((data) => {
        const userData = data.map((item) => ({
          label: item.username,
...
<div className='Container ' style={{borderColor:"#C67D6B", overflow: "auto"}}>
 ...
 <div  style={{marginRight:"10px",width:"230px"}}> 
    ...
    <div className='divdinner' style={{display:chekInter }}>  
     <div className='divflex'   >
   <div style={{marginLeft:"-70px"}} >    <input  className='divchekbox' type="checkbox"  checked={checkEng}    onChange={handleEng}/></div>
   <div className='divflexlabel' style={{width:"300px"}}> <label  > Engineering concept</label></div>

   </div>
  <div className='divflex' >
  <div style={{marginLeft:"-70px"}}>  <input type="checkbox" className='divchekbox'  checked={checkAuilleur}    onChange={handleAuilleur}/></div>
...
<div style={{display:chekdiscontinu}} className='tabledecore'> 
    ...
    <div className='Container ' style={{borderColor:"#E3479A"}}>
      ...
      <div className='divdeuxforthree'>  <p  >{NumDevis}    </p> </div>
      <div>  <p > </p></div>
      </div> 
      <br/> 
    <div className='divforthree'> 
 ...
 <div className='lebelTextPrix'>  
   ...
   <div className='lebelText' >  <label className='textLabel' > Prénom :</label>
     ...
     {assujtTvFormateura==true? <div className='lebelText'>  <label className='textLabel' > Assujetti à la TVA :</label>
      ...
      {fraisFormateur.filter(item => item.intitile =="solidworks").map(filteredPerson => (
    <p>
      {filteredPerson.prix}
    </p>
  ))}</div>:null}
