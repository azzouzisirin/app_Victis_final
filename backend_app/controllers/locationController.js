...
exports.Register = async (req, res) => {
    try { 
        const { email,portable,observation,nom,prenom,tel,adresse_1, adresse_2,codeVille,raisonSociale,frais} = req.body;
        const newlocation = await location.create({email,portable,observation,nom,prenom,tel,adresse_1, adresse_2,codeVille,raisonSociale,frais});
...
exports.getlocationByRaisonSociale = async (req, res) => { 
    try {
        ...
        res.status(200).json({frais:Location.frais,adresse_1:Location.adresse_1});
    } catch (error) {
        res.status(400).json({
            status: 'failed',
            error
